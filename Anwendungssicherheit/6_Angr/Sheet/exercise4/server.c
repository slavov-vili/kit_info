...
int main(int argc, char** argv, char** envp) {
	...
	// Move to a new network namespace.
	...
	// Switching to a new network namespaces requires the CAP_SYS_ADMIN capability, which we might not have.
	// However, we can get that capability by first moving into a new user namespace.
	{

		uid_t my_uid = getuid();
		gid_t my_gid = getgid();

		// move to a new user and network namespace
		result = unshare(CLONE_NEWUSER | CLONE_NEWNET);
		if (result != 0) {
			die("failed to move to a new user and network namespace");
		};

		// we now need to initialize the new user namespace.

		// initialize the /proc/self/uid_map file
		char mapping[32];
		result = snprintf(mapping, sizeof(mapping), "%u %u 1\n", my_uid, my_uid);
		if (result >= sizeof(mapping) || result < 0) {
			die("failed to create uid_mapping");
		};
		immediate_write_to_file("/proc/self/uid_map", mapping);

		// initialize /proc/self/setgroups and /proc/self/gid_map file
		immediate_write_to_file("/proc/self/setgroups", "deny");
		result = snprintf(mapping, sizeof(mapping), "%u %u 1\n", my_gid, my_gid);
		if (result >= sizeof(mapping) || result < 0) {
			die("failed to create gid_mapping");
		};
		immediate_write_to_file("/proc/self/gid_map", mapping);

	}

	// After moving to a new user namespace, we now have a full set of capabilities.
	// Drop them.
	{
